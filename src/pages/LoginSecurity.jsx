...
export default function LoginSecurity() {
  ...
  useEffect(() => {
    if (user) {
      if (user.role === 'security') navigate('/security-dashboard', { replace: true })
      else if (isCEO(user)) navigate('/ceo-dashboard', { replace: true })
  ...
  const onSubmit = async (e) => {
    ...
    setError('')
    setLoading(true)
    try { await login({ email, password, roleHint: 'security' }) }
        ...
        <div className="card-body">
          ...
          <form onSubmit={onSubmit}>
            ...
            <button className="btn btn-warning w-100" type="submit" disabled={loading}>
              {loading ? <><i className="fas fa-spinner fa-spin me-2"></i>Signing in...</> : 'Login'}
            </button>
          </form>
          <div className="text-center mt-3">
            <Link to="/forgot-password?portal=security" className="text-decoration-none">Forgot your password?</Link>
          </div>
          <div className="text-center mt-2">
            <small className="text-muted">
              Other portals: <Link to="/staff" className="text-decoration-none ms-1">Staff</Link> | <Link to="/admin" className="text-decoration-none ms-1">Admin</Link> | <Link to="/ceo" className="text-decoration-none ms-1">CEO</Link>
            </small>
          </div>
        </div>
      </div>
